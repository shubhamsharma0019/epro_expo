...
return new class extends Migration
{
        ...
        Hall::query()
            ...
            ->each(function (Hall $hall) use ($now) {
                ...
                foreach ($bookings as $booking) {
                    $requiredSpaces = $this->requiredSpacesForBooking($booking);

                    $assignment = SequentialBoothAllocation::assignBlockForBooking(
                        $hallBooths,
                        $requiredSpaces,
                        preferredStartIndex: null,
                        alreadyAssignedBoothIds: $assignedBoothIds,
                        packFromStart: true,
                    );
    ...
    private function requiredSpacesForBooking(BoothBooking $booking): int
    {
        if ($booking->booth_size_id) {
            $size = $booking->relationLoaded('boothSize')
                ? $booking->boothSize
                : BoothSize::query()->find($booking->booth_size_id);

            if ($size) {
                return max(BoothFloorMap::unitsForSize($size), 1);
            }
        }
