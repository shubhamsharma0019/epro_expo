        ...
        @forelse ($sessions as $session)
            ...
            <article class="flex min-h-[270px] flex-col rounded-xl border border-[#E7EAF3] bg-white p-5 shadow-[0_10px_28px_rgba(7,16,68,0.07)]">
                ...
                <div class="mt-auto flex flex-col gap-3 pt-5 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ $joinHref }}" class="inline-flex h-11 items-center justify-center rounded-md {{ $isPassActive ? 'bg-[#5b2eff] text-white' : 'border border-[#EADCFD] bg-[#FBFAFF] text-purple' }} px-5 text-[13px] font-semibold">
                        {{ $isPassActive ? 'Join Session' : 'Get Pass' }}
                    </a>
                </div>
            </article>
        @empty
