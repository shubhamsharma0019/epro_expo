...
@section('title', 'QR Ticket - ' . ($ticket->companyEvent?->title ?? 'Event'))
...
<main class="mx-auto w-full max-w-[1080px] flex-1 px-4 pb-12 pt-6 md:px-[32px]">
    <a href="{{ route('events.tickets.confirmed', ['order' => $ticket->order_number]) }}"
                ...
                <div class="relative z-10">
                    ...
                    <div class="mt-8 grid gap-3 sm:grid-cols-2">
                        <div class="rounded-xl border border-white/14 bg-white/10 p-4">
                            <p class="text-[12px] text-white/60">Attendee</p>
                            <p class="mt-1 text-[15px] font-bold">{{ $ticket->attendee_name }}</p>
                        </div>
                        <div class="rounded-xl border border-white/14 bg-white/10 p-4">
                            <p class="text-[12px] text-white/60">Ticket Type</p>
                            <p class="mt-1 text-[15px] font-bold">{{ $ticket->ticket_name }} × {{ $ticket->quantity }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <form method="POST" action="{{ route('events.tickets.send-ticket-email') }}">
            @csrf
            ...
            <button type="submit" class="inline-flex h-[52px] w-full items-center justify-center rounded-xl border border-[#DDE2F2] bg-white px-5 text-[15px] font-bold text-[#071044] transition hover:bg-[#F8FAFF]">
                {{ ($emailSent ?? false) ? 'Resend Email' : 'Send to Email' }}
            </button>
        </form>
    </div>
</main>
