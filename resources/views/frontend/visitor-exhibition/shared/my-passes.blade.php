        ...
        <div class="overflow-hidden rounded-lg border border-borderColor bg-white shadow-sm">
            <div class="visitor-flow-scroll-tabs flex items-center">
                @foreach (['All Passes' => 'all', 'Active' => 'active', 'Upcoming' => 'upcoming', 'Completed' => 'completed'] as $tab => $filter)
                    <button type="button" 
                            data-filter="{{ $filter }}" 
                            class="pass-tab-btn h-[56px] {{ $filter === 'all' ? 'border-b-2 border-purple text-purple' : 'text-[#34405F]' }} px-8 text-[15px] font-semibold focus:outline-none cursor-pointer transition-all shrink-0 whitespace-nowrap">
                        {{ $tab }}
                    </button>
                @endforeach
            </div>
    ...
    <div class="space-y-5">
        @if(isset($visitors) && $visitors->count() > 0)
            @foreach ($visitors as $visitor)
                @php
                    ...
                    $bookmarksCount = \App\Domain\Visitor\Models\Bookmark::where('booking_id', $visitor->booking_id)->where('bookmarkable_type', 'exhibitor')->count();
        ...
        @else
            <div class="visitor-flow-empty">
                <p class="text-[16px] font-semibold text-navy">No visitor passes found</p>
                <p class="mt-2 text-[14px] text-[#5A6480]">Register for this exhibition to see your passes here.</p>
                @if ($slug)
                    <a href="{{ route('exhibitions.tickets.select', $slug) }}" class="visitor-flow-btn mt-5 inline-flex h-11 items-center justify-center rounded-md bg-gradient-to-r from-[#5b2eff] to-[#4310d8] px-5 text-[14px] font-semibold text-white">Get Visitor Pass</a>
                @endif
            </div>
        @endif
