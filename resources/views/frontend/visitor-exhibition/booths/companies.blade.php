...
@php
    ...
    $industryFilters = collect(['All']);
    if (isset($booths)) {
        $industryFilters = $industryFilters->merge(
            $booths->map(fn ($booking) => $booking->boothProfile?->industry ?: $booking->company?->industry)->filter()->unique()->values()
        );
    }
@endphp
    ...
    <div class="mb-6 overflow-hidden rounded-[14px] border border-[#E7EAF3] bg-white shadow-[0_10px_28px_rgba(7,16,68,0.07)]">
        <div class="grid gap-0 xl:grid-cols-[minmax(0,1fr)_310px]">
            <div class="p-5 lg:p-6">
                ...
                <h1 class="text-[26px] font-black leading-[32px] tracking-[-0.9px] text-navy sm:text-[42px] sm:leading-[46px]">
                ...
                <div class="mt-5 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                    ...
                    <div class="visitor-flow-scroll-tabs flex flex-wrap gap-2">
                        @foreach ($industryFilters as $filter)
                            <button type="button" class="h-[38px] shrink-0 rounded-full border border-borderColor bg-[#FBFAFF] px-4 text-[12px] font-semibold text-[#34405F] hover:border-purple hover:bg-[#F4F0FF] hover:text-purple">
                                {{ $filter }}
                            </button>
                        @endforeach
                    </div>
