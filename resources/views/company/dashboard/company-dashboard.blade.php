...
<section class="mx-auto w-full min-w-0 max-w-[1400px] p-4 sm:p-6 lg:p-8">
    @if ($pendingBooking)
    ...
    <div class="mb-8 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
            <h2 class="mb-1 flex items-center gap-2 text-[22px] font-bold leading-tight text-gray-900 sm:text-[28px]">
    ...
    <div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 xl:grid-cols-4">
        <div class="flex min-w-0 flex-col justify-between rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
    ...
    <div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 xl:grid-cols-4">
        ...
        <div class="flex min-w-0 flex-col justify-between rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
            ...
            <h3 class="break-words text-xl font-bold leading-none text-gray-900 sm:text-[28px]">&#8377;{{ number_format($stats['total_spend'] ?? 0, 2) }}</h3>
    ...
    <div class="mb-8 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
        <div class="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            ...
            <a href="{{ url('/company/exhibitions') }}" class="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-blue-700 sm:w-auto">
            ...
            <div class="relative mb-8 mt-2 px-0 py-2 sm:px-10">
                ...
                <div class="relative z-10 flex flex-col items-stretch justify-between gap-4 sm:flex-row sm:items-center sm:gap-0">
                    @foreach ([
                        ...
                        <div class="flex w-full items-center gap-3 sm:w-auto sm:flex-col sm:gap-3 sm:text-center">
                            <div class="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full font-bold shadow-sm ring-4 ring-white sm:h-16 sm:w-16 sm:ring-8 {{ $active ? ($state === 'success' ? 'bg-success-light text-success' : ($state === 'warning' ? 'bg-warning-light text-warning' : 'bg-primary-light text-primary')) : 'bg-gray-50 text-gray-400' }}">
                                <i class="{{ $icon }} text-2xl sm:text-4xl"></i>
                                @if ($active)
                ...
                <div class="absolute inset-0 flex flex-col justify-between pb-8">
                    @foreach ($performanceAxisLabels->push(0) as $label)
                        <div class="flex w-full items-center">
                            <span class="mr-2 w-8 text-right text-[10px] font-medium text-gray-400 sm:mr-4 sm:w-10 sm:text-xs">{{ number_format($label) }}</span>
                            <div class="flex-1 border-b border-gray-50"></div>
                        </div>
                    @endforeach
                </div>

                <div class="absolute inset-0 top-2 h-[210px] w-full pb-8 pl-10 pr-2 sm:h-[250px] sm:pl-14 sm:pr-4">
