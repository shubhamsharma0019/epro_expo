...
<div class="w-full mx-auto max-w-[1400px]">
            ...
            <div class="flex flex-col gap-6 md:flex-row md:justify-between md:items-start md:gap-8 mb-10">
                <div class="flex items-start">
                    <!-- Booth Icon -->
                    <div class="mt-1 mr-4 text-[#3D1B9B] shrink-0">
                        <svg class="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 3L4 9v11a1 1 0 001 1h4v-6h6v6h4a1 1 0 001-1V9l-8-6z"/>
                        </svg>
                    </div>
                    <div>
                        <h1 class="text-[24px] sm:text-[28px] font-bold text-[#1E1B4B] mb-2 tracking-tight">Setup Your Booth</h1>
                        <p class="text-[#6B7280] text-[14px] sm:text-[15px]">Complete all the steps below to publish your booth and go live.</p>
                    </div>
                </div>
                
                <!-- Overall Progress Card -->
                <div class="w-full md:w-[360px] border border-gray-200 rounded-xl p-4 shadow-sm bg-white shrink-0">
                    <div class="flex justify-between items-center mb-3">
                        <span class="text-[14px] sm:text-[15px] font-bold text-[#1E1B4B]">Overall Progress</span>
                        <span class="text-[14px] sm:text-[15px] font-bold text-[#1E1B4B]" id="overall-progress-text">{{ $progress ?? 0 }}%</span>
                    </div>
            ...
            <div class="border border-gray-200 rounded-2xl bg-white mb-10 flex flex-col overflow-hidden" id="steps-list-container">
                @foreach ($steps as $step)
                    ...
                    @endphp
                    <a href="{{ $links[$step->step_key] ?? '#' }}" class="flex flex-col gap-3 border-b border-gray-100 px-6 py-5 last:border-b-0 hover:bg-[#FBFAFF] sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex min-w-0 items-center gap-4">
                            <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#F4F0FF] text-[#3D1B9B] font-bold">{{ $step->sort_order }}</span>
                            <div class="min-w-0">
                                <h3 class="break-words text-[#1E1B4B] font-bold text-[16px]">{{ $step->step_name }}</h3>
                                <p class="text-[#6B7280] text-[13px]">Connected to booking #{{ $booking->id }}</p>
                            </div>
                        </div>
                        <span class="shrink-0 self-start rounded-md border px-3 py-1 text-[12px] font-bold sm:self-center {{ $statusClasses[$step->status] ?? $statusClasses['pending'] }}">
            ...
            <div class="flex flex-col-reverse gap-4 sm:flex-row sm:justify-between sm:items-center sm:gap-0 pb-12">
                <a href="{{ route('company.bookings.show', $booking) }}" class="flex items-center justify-center sm:justify-start text-[#3D1B9B] font-bold text-[15px] hover:underline transition-all py-2">
                ...
                <a href="{{ route('company.booth-setup.profile.edit', $booking) }}" class="px-8 py-3.5 bg-[#3D1B9B] rounded-lg text-white font-bold text-[15px] hover:bg-[#31167D] transition-colors inline-flex items-center justify-center shadow-md">
