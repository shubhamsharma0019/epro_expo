...
<section class="px-4 py-6 sm:px-6 lg:px-8">
    <div class="mx-auto w-full max-w-[1400px] rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
        <h1 class="mb-2 text-[28px] font-bold tracking-tight text-[#1E1B4B]">Upload Catalogues</h1>
        <p class="mb-8 text-[15px] text-[#6B7280]">Upload your digital catalogues and lookbooks.</p>
        ...
        <form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="mb-10">
            ...
            <label class="mb-5 flex w-full cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#8B5CF6] bg-white py-12 text-center">
                <span class="mb-4 text-[#3D1B9B]">
                    <svg class="h-16 w-16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 16V4m0 0L8 8m4-4 4 4M6 20h12a2 2 0 0 0 2-2v-3m-16 3v-3a2 2 0 0 1 2-2h2"></path></svg>
                </span>
                <span class="mb-2 text-[16px] font-bold text-[#1E1B4B]">Upload catalogue PDF/file</span>
                <span id="catalogue-file-name" class="mb-6 text-[13px] text-[#6B7280]">PDF, DOC, DOCX, PPT, PPTX, XLS, XLSX (Max. 20MB)</span>
        ...
        <div class="mb-6 overflow-hidden rounded-xl border border-gray-100 bg-white">
            <div class="overflow-x-auto">
                <div class="min-w-[900px] border-b border-gray-100 p-6">
                    <h3 class="mb-6 text-[16px] font-bold text-[#1E1B4B]">Uploaded Catalogues</h3>
                ...
                @forelse ($catalogues as $item)
                    <div class="grid min-w-[900px] grid-cols-12 items-center gap-4 border-b border-gray-100 p-6 last:border-b-0">
                        ...
                        <div class="col-span-2 text-[14px] text-[#6B7280]">{{ $item->category ?: '-' }}</div>
                        <div class="col-span-1 text-center text-[14px] text-[#6B7280]">{{ $item->pages ?: '-' }}</div>
                        <div class="col-span-2 text-center text-[14px] text-[#6B7280]">{{ $item->file_size ? number_format($item->file_size / 1024 / 1024, 1) . ' MB' : '-' }}</div>
