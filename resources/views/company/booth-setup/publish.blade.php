...
@php
    ...
    $isSubmitted = in_array($publishStatus, ['pending', 'approved'], true) || in_array($booking->booth_setup_status, ['pending_review', 'published', 'approved', 'live'], true);
    $readinessCopy = $readiness['ready']
        ? 'Great! Your booth is ready to be published.'
        : 'Complete the pending requirements before publishing your booth.';
    $stepMeta = [
        ...
        'publish' => ['icon' => 'M5 13l4 4L19 7', 'copy' => 'Submit booth for admin review.'],
    ];
@endphp
            ...
            <div class="col-span-12 lg:col-span-5 border border-gray-100 rounded-xl p-6 bg-white shadow-sm flex flex-col h-full">
                ...
                <div class="flex-1 space-y-4">
                    @foreach ($stepsList->reject(fn ($step) => $step->step_key === 'publish') as $step)
                        ...
                        @endphp
                        <div class="flex items-center justify-between gap-4 pb-4 border-b border-gray-50">
            ...
            <div class="col-span-12 lg:col-span-3 flex flex-col justify-between h-full">
                ...
                <div class="mt-6">
                    ...
                    <p class="text-[#6B7280] text-[12px] text-center mt-3">
                        {{ $isSubmitted ? 'Your booth has already been submitted for review.' : 'Once published, your booth will be submitted for admin review.' }}
                    </p>
                </div>
            </div>
        </div>
