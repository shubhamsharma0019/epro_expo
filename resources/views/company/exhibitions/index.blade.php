...
<section class="max-w-[1500px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
    <div class="mb-8">
        <h1 class="text-[34px] font-semibold leading-[42px] tracking-[-0.8px] text-navy">Browse Exhibitions</h1>
        <p class="mt-3 text-[16px] font-medium leading-7 text-[#34405F]">Choose an exhibition where your company can showcase products and book a booth.</p>
    </div>
    ...
    <div class="mb-7 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <label class="relative block w-full max-w-[420px]">
            ...
            <input type="text" placeholder="Search exhibitions..." class="h-[52px] w-full rounded-md border border-borderColor pl-11 pr-4 text-[14px] font-medium outline-none placeholder:text-[#6B7280] focus:border-purple">
        </label>
        <button type="button" class="inline-flex h-[52px] items-center justify-center gap-3 rounded-md border border-purple px-5 text-[15px] font-semibold text-purple">
    ...
    <div class="grid grid-cols-1 gap-7 md:grid-cols-2 xl:grid-cols-3">
        @forelse ($exhibitions as $exhibition)
            @php
                $pavTitle = optional($exhibition->pavilions->first())->title ?? 'Innovation Pavilion';
            ...
            <article class="overflow-hidden rounded-xl border border-borderColor bg-white shadow-sm">
                <img src="{{ $imageUrl }}" alt="{{ $exhibition->title }}" class="h-[168px] w-full object-cover">
                <div class="p-6">
                    <h2 class="text-[21px] font-semibold text-navy">{{ $exhibition->title }}</h2>
                    ...
                    <div class="mt-6 flex items-center justify-between gap-4">
                        @if ($exhibition->user_booking_status === 'booked')
                            ...
                            <a href="{{ route('company.dashboard') }}" class="inline-flex h-[38px] items-center justify-center rounded-md bg-primary px-4 text-xs font-semibold text-white">
                                Setup Booth
                            </a>
                        ...
                        @else
                            ...
                            <a href="{{ url('/company/exhibitions/' . $exhibition->slug) }}" class="inline-flex h-[38px] items-center justify-center rounded-md border border-purple px-4 text-xs font-semibold text-purple hover:bg-purple/5">
                                Book Now
                            </a>
                        @endif
                    </div>
