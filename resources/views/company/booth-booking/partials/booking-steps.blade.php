@php
    ...
    $steps = [
        ...
        [5, 'Summary', '/company/booth-booking/summary'],
        [6, 'Services', '/company/booth-booking/services'],
        [7, 'Review', '/company/booth-booking/review'],
        [8, 'Payment', '/company/booth-booking/payment'],
        [9, 'Confirmed', '/company/booth-booking/confirmed'],
    ];
@endphp
...
<div class="mb-7 overflow-hidden rounded-2xl border border-[#E7EAF3] bg-white px-4 py-4 shadow-sm">
    <div class="flex items-center gap-3 overflow-x-auto pb-1 text-[13px] font-semibold text-[#34405F]">
        @foreach ($steps as [$step, $label, $href])
            ...
            <a href="{{ $url }}" class="flex shrink-0 items-center gap-2 rounded-full px-2 py-1 {{ $isActive ? 'bg-[#F4F0FF] text-[#5b2eff]' : '' }}">
