...
@php
    ...
    $selectedCompanyName = $currentCompany?->company_name ?? $currentCompany?->name ?? 'Your company';
...
<section class="flex min-h-[calc(100dvh-4rem)] w-full max-w-none flex-col px-4 py-4 sm:px-6 sm:py-5 lg:px-8">
    <div class="mb-4 flex shrink-0 flex-col gap-4 lg:mb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
    ...
    <div class="relative z-[1] grid min-h-0 flex-1 grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        <div class="flex min-h-0 min-w-0 flex-col">
            <div id="floor-map-shell" class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-borderColor bg-white p-3 shadow-sm sm:p-4">
                <div class="mb-3 flex shrink-0 items-center gap-4 px-2 text-center text-[15px] font-semibold text-navy sm:gap-8 sm:px-4 sm:text-[16px]">
                    <div class="h-px flex-1 bg-[#9AA3B8]"></div>
                    <div class="shrink-0">Main Aisle</div>
                    <div class="h-px flex-1 bg-[#9AA3B8]"></div>
                </div>

                <div id="floor-map-viewport" class="flex min-h-[320px] w-full flex-1 items-center justify-center overflow-hidden sm:min-h-[420px] lg:min-h-[calc(100dvh-22rem)]">
                    ...
                    <div class="relative w-full min-h-[400px] rounded-md border border-[#BFC8DE] bg-white">
                        ...
                        @forelse ($booths as $booth)
                            ...
                            <button
                                ...
                                data-select-url="{{ url('/company/booth-booking/summary?' . http_build_query(array_filter(['exhibition' => session('company_booth_booking.exhibition_slug')]))) }}"
                        ...
                        @foreach ($selectedSpaceSegments as $segmentIndex => $segment)
                            <div
                                ...
                                class="absolute z-20 flex flex-col items-center justify-center gap-1 rounded-md bg-[#4B18D9] px-1.5 py-1 text-center font-bold text-white shadow-md ring-1 ring-black/10"
                            >
                                @if ($segmentIndex === 0)
                                    <span class="flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-full bg-white/95 text-[11px] font-extrabold text-[#4B18D9]">
                                        @if ($companyLogoUrl)
                                            <img src="{{ $companyLogoUrl }}" alt="{{ $selectedCompanyName }}" class="h-full w-full object-cover" onerror="this.remove(); this.parentElement.textContent='{{ $companyInitial }}';">
                                        @else
                                            {{ $companyInitial }}
                                        @endif
                                    </span>
                                    <span id="selected-space-label" class="{{ \App\Support\HallBookedBoothGroups::overlayLabelClass(max((int) ($segment['width'] ?? 48), 48), max((int) ($segment['height'] ?? 44), 44)) }} text-white" title="{{ $selectedCompanyName }}">{{ $selectedCompanyName }}</span>
                                @endif
                            </div>
                        @endforeach
                        ...
                        @foreach ($bookedBoothGroups as $groupIndex => $group)
                            ...
                            @foreach ($groupSegments as $segmentIndex => $segment)
                                ...
                                <button
                                    ...
                                    style="position: absolute; left: {{ (int) ($segment['left'] ?? 0) }}px; top: {{ (int) ($segment['top'] ?? 0) }}px; width: {{ $segmentWidth }}px; height: {{ $segmentHeight }}px;"
                                    class="absolute z-40 flex cursor-pointer flex-col items-center justify-center gap-1 rounded-md bg-[#777777] px-1.5 py-1 text-center font-bold text-white shadow-md ring-1 ring-black/10 transition hover:bg-[#5f5f5f]"
            ...
            <div class="mt-4 shrink-0 rounded-xl border border-borderColor bg-white px-4 py-4 shadow-sm sm:mt-5 sm:px-6 sm:py-5">
...
<script>
    (() => {
        ...
        const selectedCompanyName = @json($selectedCompanyName);
        let scale = 1;
        let fitScale = 1;
        ...
        const applyScale = () => {
            if (canvas && viewport && scaleWrap) {
                ...
                scaleWrap.style.width = `${baseCanvasWidth * finalScale}px`;
                scaleWrap.style.height = `${baseCanvasHeight * finalScale}px`;
        ...
        const updateFitScale = () => {
            ...
            const widthScale = viewport.clientWidth / baseCanvasWidth;
            const heightScale = viewport.clientHeight / baseCanvasHeight;
            fitScale = Math.max(0.5, Math.min(widthScale, heightScale));
            applyScale();
        };
