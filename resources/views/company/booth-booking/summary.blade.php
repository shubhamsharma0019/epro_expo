    ...
    <div class="grid grid-cols-1 gap-6 sm:gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside class="rounded-xl border border-borderColor bg-white p-4 sm:p-6 shadow-sm xl:self-start">
            ...
            <div class="mt-6 sm:mt-7 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-1">
                <a href="{{ url('/company/booth-booking/floor-plan?' . http_build_query(array_filter(['hall' => $booking->hall_id, 'booth' => $booking->booth_id, 'size' => $booking->booth_size_id, 'exhibition' => session('company_booth_booking.exhibition_slug')]))) }}" class="inline-flex h-[52px] sm:h-[58px] xl:h-[52px] w-full items-center justify-center rounded-md border border-[#B9A7FF] px-6 text-[16px] font-semibold text-purple">
                    Change Booth
                </a>
