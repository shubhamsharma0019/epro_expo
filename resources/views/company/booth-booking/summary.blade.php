    ...
    <div class="grid grid-cols-1 gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div class="rounded-xl border border-borderColor bg-white p-6 shadow-sm sm:p-8">
            <div class="mb-8 flex items-center justify-between gap-5">
                ...
                <span class="rounded-md bg-[#EAF9F0] px-3 py-1.5 text-[13px] font-semibold text-[#16A34A]">
                    {{ ucfirst($booking->booking_status) }}
                </span>
            </div>

            <div class="mb-6 grid grid-cols-1 gap-4 rounded-xl border border-[#DCD3FF] bg-[#FBFAFF] p-5 sm:grid-cols-[180px_minmax(0,1fr)] sm:items-center">
                <div>
                    <p class="text-[13px] font-semibold text-[#5A6480]">Selected Days</p>
                    <p class="mt-2 text-[38px] font-semibold leading-none text-purple">{{ $summary->selected_days_count }}</p>
                </div>
        ...
        <aside class="rounded-xl border border-borderColor bg-white p-6 shadow-sm xl:self-start">
            ...
            <div class="flex items-center justify-between gap-5">
                ...
                <span class="text-[30px] font-semibold leading-none text-navy">&#8377;{{ number_format((float) $summary->total_amount) }}</span>
            </div>
            <div class="mt-7 grid grid-cols-1 gap-3">
                <a href="{{ url('/company/booth-booking/slots?' . http_build_query(array_filter(['hall' => $booking->hall_id, 'booth' => $booking->booth_id, 'size' => $booking->booth_size_id]))) }}" class="inline-flex h-[52px] w-full items-center justify-center rounded-md border border-[#B9A7FF] px-6 text-[16px] font-semibold text-purple">
                    Edit Days
                </a>
                <a href="{{ url('/company/booth-booking/services') }}" class="inline-flex h-[58px] w-full items-center justify-center gap-4 rounded-md bg-gradient-to-r from-[#5b2eff] to-[#4310d8] px-8 text-[18px] font-semibold text-white shadow-[0_10px_20px_rgba(91,46,255,0.18)]">
                    Add Services
