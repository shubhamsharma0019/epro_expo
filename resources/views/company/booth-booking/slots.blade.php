...
@php
    ...
    $bookingDaysCount = $adminBoothDays ?? $bookingDaysCount ?? max($selectedSlots->count(), 1);
@endphp
...
<section class="max-w-[1500px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
    ...
    <div class="mb-7 rounded-xl border border-[#DCD3FF] bg-white px-5 py-5 shadow-sm sm:px-6">
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            <div>
                <h2 class="text-[20px] font-semibold text-navy">Exhibition Booth Days</h2>
                <p class="mt-2 text-[15px] font-medium leading-6 text-[#34405F]">
                    Booth booking duration is configured by the exhibition admin. All assigned days are included automatically in your booking amount.
                </p>
                <div class="mt-4 flex flex-wrap gap-3 text-[14px] font-semibold text-[#34405F]">
                    <span class="rounded-md bg-[#F4F0FF] px-3 py-2 text-purple">Admin days: {{ $bookingDaysCount }}</span>
                    <span class="rounded-md bg-[#F4F0FF] px-3 py-2 text-purple">Per day: &#8377;{{ number_format((float) $slotPrice) }}</span>
                    <span class="rounded-md bg-[#FBFCFF] px-3 py-2">Included: {{ $selectedSlots->count() }} {{ $selectedSlots->count() === 1 ? 'day' : 'days' }}</span>
    ...
    <div class="mb-5 flex flex-col gap-3 text-[15px] font-semibold text-[#34405F] lg:flex-row lg:items-center lg:justify-between">
        ...
        <div>Every included day adds &#8377;{{ number_format((float) $slotPrice) }}</div>
    </div>
                        ...
                        <tr class="border-b border-borderColor last:border-b-0">
                            ...
                            @foreach ($group['slots'] as $slot)
                                ...
                                <td class="border-l border-borderColor px-8 py-4 text-center">
                                    @if ($isBooked)
                                        <span class="inline-flex h-[44px] min-w-[130px] items-center justify-center rounded-md bg-[#E6E8EF] px-6 text-[#6B7280]">
                                            Booked
                                        </span>
                                    @elseif ($isSelected)
                                        <span class="inline-flex h-[44px] min-w-[130px] items-center justify-center gap-3 rounded-md bg-gradient-to-r from-[#5b2eff] to-[#4310d8] px-6 text-white">
                                            <i class="fa-solid fa-check text-[13px]"></i>
                                            Included
                                        </span>
                                    @else
                                        <span class="inline-flex h-[44px] min-w-[90px] items-center justify-center rounded-md px-5 text-[#9AA3B8]">
                                            Not included
                                        </span>
                                    @endif
                                </td>
    ...
    <div class="mt-7 rounded-xl border border-borderColor bg-white px-5 py-5 shadow-sm sm:px-6">
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div>
                <h2 class="text-[18px] font-semibold text-navy">Included Days ({{ $selectedSlots->count() }})</h2>
                <div class="mt-3 flex flex-wrap gap-2">
                    ...
                    @empty
                        <span class="text-[14px] font-semibold text-[#5A6480]">No days included yet.</span>
                    @endforelse
                </div>
