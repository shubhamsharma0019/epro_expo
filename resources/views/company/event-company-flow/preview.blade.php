...
@php
    ...
    $eventTagline = $eventBranding?->tagline ?: $eventBranding?->headline ?: $companyEvent->summary ?: 'Event tagline will appear here.';
    ...
    $eventAddress = collect([$companyEvent->venue_address, $companyEvent->city, $companyEvent->country])->filter()->join(', ');
    ...
    $ctaUrl = $eventBranding?->cta_url ?: '#tickets';
    $tabs = collect([
...
<div class="px-10 py-8 max-w-[1250px] w-full flex flex-col mx-auto flex-1 pb-20">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-8">
            <a href="{{ route('company.event-company-flow.submit', $companyEvent) }}" style="border-color: {{ $primaryColor }}; color: {{ $primaryColor }};" class="bg-white border px-5 py-2 rounded-lg text-[13px] font-bold flex items-center gap-2 hover:bg-[#F4F1FF] transition-colors shadow-sm">
                Continue Review
    ...
    <div class="relative w-full h-[280px] rounded-[16px] overflow-hidden mb-6 shadow-sm bg-cover bg-center bg-no-repeat" style="background-color: {{ $primaryColor }}; @if ($bannerUrl) background-image: url('{{ $bannerUrl }}'); @endif">
        @unless ($bannerUrl)
        ...
        <div class="absolute inset-0 p-10 flex flex-col justify-center">
            <div class="text-white mb-6 max-w-[760px]">
                ...
                <h1 class="text-[32px] font-bold tracking-tight mb-2">{{ str($eventTitle)->upper() }}</h1>
        ...
        <div class="flex flex-col gap-8">
            <div id="about" class="bg-white border border-gray-100 rounded-[16px] p-8 shadow-[0_2px_10px_rgba(0,0,0,0.01)] h-fit">
                <div class="flex flex-wrap gap-2 mb-5">
                    <span class="rounded-full bg-[#F4F1FF] px-3 py-1 text-[12px] font-bold" style="color: {{ $primaryColor }}">{{ $eventModeLabel }}</span>
