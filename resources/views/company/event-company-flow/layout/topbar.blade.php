@php
    ...
    $eventFlowNotificationCount = $eventFlowCompany
        ? app(\App\Domain\Company\Services\CompanyNotificationService::class)->unreadCount($eventFlowCompany)
        : 0;
@endphp
...
<header class="z-40 flex h-[72px] shrink-0 min-w-0 items-center justify-between gap-4 border-b border-gray-100 bg-white px-4 sm:px-6 lg:hidden">
    ...
    <div class="min-w-0 shrink-0 flex items-center gap-2 sm:gap-3">
        ...
        <a href="{{ route('company.notifications', ['flow' => 'event']) }}" id="company-notification-bell" class="relative grid h-9 w-9 place-items-center rounded-xl text-gray-500 transition-colors hover:bg-[#F8F9FA] hover:text-[#1C1364]">
            <i class="ph ph-bell text-2xl"></i>
            <span
                id="company-notification-badge"
                class="absolute right-0.5 top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full border-2 border-white bg-red-500 px-1 text-[9px] font-bold leading-none text-white {{ $eventFlowNotificationCount > 0 ? '' : 'hidden' }}"
                data-unread-count="{{ $eventFlowNotificationCount }}"
            >{{ $eventFlowNotificationCount > 99 ? '99+' : $eventFlowNotificationCount }}</span>
        </a>

        <a href="{{ route('company.profile', ['flow' => 'event']) }}" class="flex min-w-0 items-center text-gray-900 no-underline">
            <span class="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full border border-gray-100 bg-gray-100 text-sm font-bold text-[#5B32F6]">
                @if ($eventFlowLogoUrl)
