...
<section class="px-4 py-6 sm:px-6 lg:px-8">
    <div class="mx-auto w-full max-w-[1400px] rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6 lg:p-8">
        <h1 class="mb-6 text-[22px] font-bold tracking-tight text-[#1E1B4B] sm:text-[28px] lg:mb-8">Upload Documents</h1>
        ...
        <form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="mb-10">
            ...
            <label class="mb-5 flex w-full cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-[#8B5CF6] bg-white px-4 py-8 text-center sm:py-12">
                <span class="mb-4 text-[#3D1B9B]">
                    <svg class="h-12 w-12 sm:h-16 sm:w-16" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                </span>
                <span class="mb-2 text-[15px] font-bold text-[#1E1B4B] sm:text-[16px]">Drag & drop files here or <span class="text-[#3D1B9B]">click to browse</span></span>
                <span id="document-file-name" class="mb-6 text-[12px] text-[#6B7280] sm:text-[13px]">PDF, DOC, DOCX, PPT, PPTX, XLS, XLSX, JPG, PNG (Max. 20MB)</span>
        ...
        <div class="mb-6 overflow-x-auto rounded-xl border border-gray-100 bg-white">
            <div class="lg:min-w-[900px]">
                ...
                @forelse ($documents as $item)
                    @php
                        $documentPath = ltrim((string) $item->file_path, '/');
                        $documentUrl = $documentPath ? asset(str_starts_with($documentPath, 'storage/') ? $documentPath : 'storage/' . $documentPath) : '#';
                        $isImageDocument = in_array(strtolower((string) $item->file_type), ['jpg', 'jpeg', 'png', 'webp']);
                    @endphp
                    <div class="flex flex-col gap-3 border-b border-gray-100 p-4 last:border-b-0 lg:grid lg:grid-cols-12 lg:items-center lg:gap-4 lg:p-5">
                        <div class="flex min-w-0 items-center lg:col-span-4">
                            @if ($isImageDocument && $documentPath)
                                <img src="{{ $documentUrl }}" alt="{{ $item->title }}" class="mr-3 h-10 w-14 flex-shrink-0 rounded-md border border-gray-100 object-cover">
                            @else
                        ...
                        <div class="text-[14px] text-[#4B5563] lg:col-span-2">
                            <span class="mr-2 font-semibold text-[#1E1B4B] lg:hidden">Type:</span>{{ $item->document_type ?? strtoupper($item->file_type ?? 'file') }}
                        </div>
                        ...
                        <div class="text-[14px] text-[#4B5563] lg:col-span-1 lg:text-center">
                            <span class="mr-2 font-semibold text-[#1E1B4B] lg:hidden">Downloads:</span>{{ number_format($item->downloads ?? 0) }}
                        </div>
                        <div class="flex gap-2 lg:col-span-1 lg:justify-center">
                            <a href="{{ $documentUrl }}" target="_blank" class="flex h-8 w-8 items-center justify-center rounded border border-gray-200 text-gray-500 hover:bg-gray-50" title="View/download"><i class="ph ph-download-simple"></i></a>
