<div class="rounded-xl border border-blue-200 bg-blue-50 px-5 py-4 text-[15px] text-blue-900">
    <p class="font-semibold">One-time sender setup - then tickets go automatically</p>
    <ul class="mt-2 list-disc space-y-1 pl-5">
        <li><strong>Sender email</strong> = the email address used to send all ticket emails.</li>
        <li><strong>Buyer email</strong> = the ticket goes to the email entered during checkout.</li>
        <li>No buyer password is used for email delivery. Sender authorization is saved once in MySQL.</li>
    </ul>
</div>
...
@endif
@if (session('error') && ! $isDeliverable)
...
@if ($isDeliverable)
    <div class="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-emerald-800">
        ...
        <p class="mt-1 text-sm">Use <strong>Send Email</strong> / <strong>Resend Email</strong> on the ticket page as often as you need. Buyer password is never required.</p>
    </div>
@endif

<section class="mt-8 rounded-xl border border-[#E4E7EF] bg-white p-6 shadow-sm">
    <h2 class="text-xl font-semibold">Ticket Email Sender</h2>
    <p class="mt-2 text-sm text-[#5A6480]">This email is only used as the sender. Each buyer email is used automatically as the recipient.</p>

    <form method="POST" action="{{ $saveRoute }}" class="mt-6 space-y-4">
        @csrf
        <div>
            <label class="mb-1 block text-sm font-semibold">Sender Email</label>
        ...
        <div>
            <label class="mb-1 block text-sm font-semibold">Sender Authorization Code</label>
            <input type="password" name="mail_password" {{ $hasPassword ? '' : 'required' }} placeholder="sender authorization code"
                class="w-full rounded-lg border border-[#DDE2F2] px-4 py-3">
            @if ($hasPassword)
                <p class="mt-1 text-xs text-emerald-700">Sender authorization is already saved in MySQL - leave blank if you do not want to change it.</p>
            @else
                <ol class="mt-2 list-decimal space-y-1 pl-5 text-xs text-[#64748B]">
                    ...
                    <li>This is only for the sender email, never for the buyer email</li>
        ...
        <button type="submit" class="rounded-lg bg-[#4318FF] px-6 py-3 font-bold text-white">Save Sender</button>
    </form>
</section>

<section class="mt-6 rounded-xl border border-[#E4E7EF] bg-white p-6 shadow-sm">
    <h2 class="text-xl font-semibold">Test - send to buyer email</h2>
    <p class="mt-2 text-sm text-[#5A6480]">Status: {{ $isDeliverable ? 'Ready — unlimited ticket emails' : 'Save sender first' }}</p>
    @if ($exampleVisitorEmail)
        <p class="mt-1 text-xs text-[#64748B]">Latest buyer email example: {{ $exampleVisitorEmail }}</p>
    @endif
    <form method="POST" action="{{ $testRoute }}" class="mt-4 flex flex-col gap-3 sm:flex-row">
        ...
        <input type="email" name="test_recipient" value="{{ old('test_recipient', $exampleVisitorEmail) }}" required
            placeholder="Buyer email to test (e.g. buyer@gmail.com)"
