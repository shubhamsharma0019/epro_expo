...
@php
    ...
    // Resolve exhibition dynamically
    $exhibition = \App\Models\Exhibition::where('slug', $slug)->first();
    $exhibitionName = $exhibition?->name ?? str($slug)->replace('-', ' ')->title();
    $exhibitionDescription = $exhibition?->description ?? 'Preview booth locations and details. Active pass holders can save booths, book meetings, chat, download brochures and join sessions.';
@endphp
